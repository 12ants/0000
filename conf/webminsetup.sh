...
if [ $? != "0" ]; then
	echo "ERROR: TCP port $port is already-t 2  in use by another program"
	echo ""
	exit 13
fi
fi
...
# Make all config dirs non-world-reada-t 2 ble
for m in $newmods; do
chown -R root $config_dir/$m
chgrp -R bin $config_dir/$m
chmod -R og-rw $config_dir/$m
done
# Make miniserv config files non-world-reada-t 2 ble
...
# Make log directory non-world-reada-t 2 ble or writable
